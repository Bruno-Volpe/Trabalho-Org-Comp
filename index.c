[21:55, 16/10/2023] Volpe: #include <stdio.h>
#include <stdlib.h>
#include <time.h>
...
void generate_matriz(int n, int matriz[n][n]) {
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
    ...
    int zero_i = rand() % (n-2) + 1;  // Linha aleatória para o zero
    int zero_j = rand() % (n-2) + 1;  // Coluna aleatória para o zero
    matriz[zero_i][zero_j] = 0; //adiciona 0 na matriz
}

int esta_proximo_do_zero(int matriz[TAMANHO_TABULEIRO][TAMANHO_TABULEIRO], 
int linha, int coluna) {
    int vizinhos[4][2] = {
        {linh…
[21:55, 16/10/2023] Volpe: codigo semi-completo em c
[21:58, 16/10/2023] Volpe: #include <stdio.h>
#include <stdlib.h>
#include <time.h>
...
int linha, int coluna) {
    int vizinhos[4][2] = {
        {linha…
[21:58, 16/10/2023] Volpe: codigo em c com o tamanho de tabuleiro 
pre-definido
mas o resto funciona
[22:07, 16/10/2023] Volpe: #include <stdio.h>
#include <stdlib.h>
#include <time.h>
...
int main() {
    ...
    while (1) {
        char comando;
        printf("Digite 'w' para cima, 'a' para esquerda, 's' para baixo, 
'd' para direita ou 'q' para sair: ");
        scanf(" %c", &comando);
